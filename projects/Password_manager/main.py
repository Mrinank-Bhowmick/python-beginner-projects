...
def searc():
    try:
        with open("data.json","r") as f:
            ...
            messagebox.showinfo(title=f"{webinput.get()}",message=f"Email is {e} \n password is {p}")
...
def savedata():
 ...
 if emailinput.get()=="" or webinput.get()==""or passinput.get()=="":
     messagebox.showinfo(title="Warning",message="Please fill all the enteries to proceed")
 else:
    is_ok=messagebox.askokcancel(title=webinput.get(),message=f"Check all details \n Password : {passinput.get()} \n email : {emailinput.get()} ")
    if is_ok:
        try:
            with open("data.json","r") as f:
                data=json.load(f)
