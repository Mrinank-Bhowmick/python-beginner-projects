...
def game1_chooser(): #player choosing the word for hangman
    ...
    while(True):
        ...
        guess=clientSocket.recv(2048).decode().upper() #receive the guess given by player guessing it
        print("Guess=",guess)
...
def game1_guesser(): #player guessing word in hangman
    while(True):
        ...
        if("Enter" in mes):
            ...
            if("won" in guess_res.lower() or "lost" in guess_res.lower()): #if result has won or lost display it and end game by closing socket
                print(guess_res)
                clientSocket.close()
                break
            else:
...
def game2(): #Rock, paper, scissor game
    while(True):
        ...
        if(mes=="Play"):
            print(mes)
            inp=input().upper()
            while(inp!="ROCK" and inp!="PAPER" and inp!="SCISSOR"): #retry if wrong input
            ...
            if("won" in rps_score.lower() or "draw" in rps_score.lower()): #end game by closing socket
                print(rps_score)
                clientSocket.close()
                break
            else:
