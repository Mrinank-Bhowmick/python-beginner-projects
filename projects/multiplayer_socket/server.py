...
def game1_guesser(clientAddress):
    while(not attributes['break1']):
        if(attributes['checked']):
            attributes['guesser'].send("Enter your guess".encode()) #send prompt to guess word 
            print("Enter the guess")
            attributes['guess_message']=attributes['guesser'].recv(2048).decode().upper() #receive guess
            print("guess_message",attributes['guess_message'])
            attributes['chooser'].send(attributes['guess_message'].encode()) #send guess to other player
...
def game1_chooser(clientAddress,word):
    ...
    while(True):
        ...
        attributes['guesser'].send(attributes['guess_ans'].encode()) #send guess result to player
        ...
        if(count=="3" and clue==0):
            clue=1
            attributes['chooser'].send("Enter the clue".encode()) #prompt player for clue
            clue="Clue="+attributes['chooser'].recv(2048).decode().upper() #receive clue
            print("clue",clue)
...
def pre_game1(clientSocket,clientAddress):
    ...
    while(True):
        if(attributes['chooser']==""):
            ...
            word=attributes['chooser'].recv(2048).decode().upper() #receive chosen word from player
...
def game2_1(clientAddress):
    while(not attributes['break2']):
        if(attributes['mes']==""):
            attributes['player1'].send("Play".encode()) #send prompt to player1 to play
            attributes['mes']=attributes['player1'].recv(2048).decode().upper() #receive player 1's choice
...
def game2_2(clientAddress):
    while(not attributes['break2']):
        if(attributes['mes']!=""):
            attributes['player2'].send("Play".encode()) #send prompt to player2 to play
            attributes['mes']=attributes['player2'].recv(2048).decode().upper() #receive player 2's choice
            ...
            attributes['player1'].send(attributes['mes'].encode()) #send result to player1
            attributes['player2'].send(attributes['mes'].encode()) #send result to player2
            attributes['mes']=""
...
while(True):
    connectionSocket,clientAddress=serverSocket.accept() #welcome socket accepts new client 
    print("Connected to: "+clientAddress[0])
    start_new_thread(multi_client,(connectionSocket,clientAddress,)) #a thread created for each user
    thread+=1
