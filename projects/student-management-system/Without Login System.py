...
mycursor.execute("create table if not exists studentsinfo(sch_no int(50),name char(100),age int(50),email varchar(150),phone varchar(100))")
mydb.commit()
for i in mycursor:
    print(i)
...
def add_student():
    ...
    mycursor.execute("insert into studentsinfo values('"+var1+"','"+var2+"','"+var3+"','"+var4+"','"+var5+"')")
    mycursor.execute("select * from studentsinfo where Sch_no=('"+var1+"')")
    result=mycursor.fetchall()
    print(tabulate(result, headers=['Sch_no', 'Name','Age','Email','Phone'], tablefmt='psql'))
    mydb.commit()
...
def update_student():
    ...
    var4=str(input("Enter the email of the student:"))
    var5=str(input("Enter the phone no. of the student:"))
    mycursor.execute("update studentsinfo set name=('"+var1+"'),age=('"+var2+"'),email=('"+var3+"'),phone=('"+var4+"') where Sch_no=('"+var5+"')")
    mycursor.execute("select * from studentsinfo where Sch_no=('"+var1+"')")
    result=mycursor.fetchall()
    if len(result)==0:
        print("Enter valid sch no.!")    
