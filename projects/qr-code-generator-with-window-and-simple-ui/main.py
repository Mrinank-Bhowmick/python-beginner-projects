...
headingLabel = Label(headingFrame, text="Generate QR Code", bg='purple', font=('Times', 20, 'bold'))
...
label2 = Label(Frame2, text="Enter the location to save the QR Code: ", bg="purple", fg='azure',
...
label3 = Label(Frame3, text="Enter the name of the QR Code: ", bg="purple", fg='azure',
               font=('Courier', 13, 'bold'))
...
label4 = Label(Frame4, text="Enter the size from 1 to 40 with 1 being 21x21: ", bg="purple", fg='azure',
               font=('Courier', 13, 'bold'))
...
def generateCode():
    ...
    filedirec = loc.get() + '\\' + name.get()  # Getting the directory where the file has to be safe
    img.save(f'{filedirec}.png')  # Saving the QR Code
    # Showing the pop-up message on saving the file
    messagebox.showinfo("RavenyBoi's QR code generator", "QR Code is saved successfully!")
