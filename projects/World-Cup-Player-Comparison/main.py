...
def main():
  ...
  def get_team_data_links():
    ...
    for row in rows: 
      ...
      team_url = "https://fbref.com" + href_line[href_line.find("/"):href_line.find("\">")]
  ...
  def get_team_stats(url): 
    ...
    df_table = df_table.rename(columns={0: "Player", 1:"Pos", 2: "Age", 3: "MP", 4: "Starts", 5:"Min", 6:"90s", 7:"Goals", 8:"Ast"})
  ...
  if second_player not in list(second_df['Player']):
    print(f"{second_player} was not found on the {second_country}, please try again")
    exit()
  ...
  for name, goals, assists in zip(first_df['Player'], first_df['Goals'], first_df['Ast']):
    total_ga = 0
    if name == first_player:
      total_ga = int(goals) + int(assists)
      y_axis.append(total_ga)

  for name, goals, assists in zip(second_df['Player'], second_df['Goals'], second_df ['Ast']):
    total_ga = 0
