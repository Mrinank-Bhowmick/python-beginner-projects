...
#Public/private
type_of_ip = None
if (int(ip_list_octet[0]) == 10) or (int(ip_list_octet[0]) == 172 and int(ip_list_octet[1]) in range(16,32)) or (int(ip_list_octet[0]) == 192 and int(ip_list_octet[1]) == 168):
    type_of_ip = "Private"
else:
    type_of_ip = "Public"
...
subnet_decimal_list = []
for i in range(4):
    subnet_decimal_list.append(int(('0b' + mask_bin_list[i]), 2) & int(('0b' + bin_ip_list[i]), 2))
...
if 1 <= int(ip_prefix) <= 8:
    last_av_host = [subnet_decimal_list[0] + 2**(8-int(ip_prefix)) - 1] + [255, 255, 254]
elif 9 <= int(ip_prefix) <= 16:
    last_av_host = subnet_decimal_list[:1] + [subnet_decimal_list[1] + 2**(16-int(ip_prefix)) - 1] + [255, 254]
elif 17 <= int(ip_prefix) <= 24:
    last_av_host = subnet_decimal_list[:2] + [subnet_decimal_list[2] + 2**(24-int(ip_prefix)) - 1] + [254]
elif 25 <= int(ip_prefix) <= 30:
    last_av_host = subnet_decimal_list[:3] + [subnet_decimal_list[3] + 2**(32-int(ip_prefix)) - 2]
elif int(ip_prefix) == 31:
    last_av_host = subnet_decimal_list[:3] + [subnet_decimal_list[3] + 2**(32-int(ip_prefix)) - 1]
