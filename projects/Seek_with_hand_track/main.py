  ...
  while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
      for hand_landmarks in results.multi_hand_landmarks:
        ...
        image = cv2.circle(image, (int(x1), int(y1)), 20,  (255, 0, 0), thickness=2)
        
        landmrk2 = hand_landmarks.landmark[5]
        x2, y2 = get_coord(landmrk2, image_width, image_height)
        image = cv2.circle(image, (int(x2), int(y2)), 20,  (255, 0, 0), thickness=2)
        
        slope = np.degrees(np.arctan((landmrk1.y-landmrk2.y)/(landmrk1.x - landmrk2.x))) 
        l = np.sqrt((landmrk1.x - landmrk2.x)**2 + (landmrk1.y-landmrk2.y)**2)
        print(slope, l)
        process_inp(slope, l)
