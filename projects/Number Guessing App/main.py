...
def computer_guess():
    ...
    while CompAns != "c":
        if low != high:
            ...
            CompGuess = low  # If the range is reduced to a single value, guess that number
        
        # Ask the user if the computer's guess is too high, too low, or correct
        CompAns = input(f"Is {CompGuess} too high (h), too low (l), or correct (c)? \n=>").lower()
