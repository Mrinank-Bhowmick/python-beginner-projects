...
count_matrix=cv.fit_transform(df["combined_features"]) # feeding combined strings(movie counts) to COuntVectorizer() object
...
similar_movies=list(enumerate(cosine_sim[movie_index])) #accessing the row corresponding
