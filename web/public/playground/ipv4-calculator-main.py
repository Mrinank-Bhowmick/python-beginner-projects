# === IPv4 Calculator · annotated for the pyBegin playground ===
# A beginner-friendly walkthrough — original project by @bim22614.

# Build an IP string with its prefix notation
def pr(name, mask):
    return name + mask


# Validate that all four octets are in 0–255
...
# Ask the user for a valid IP address
ip_addr = input("Enter IP address of host: ")

# Keep asking until the address is valid
while True:
...
# Ask for the subnet prefix length
ip_prefix = int(input("Enter IP prefix: "))

# Validate that prefix is within 0–31
while True:
    if 0 <= ip_prefix <= 31:
        break
    else:
...
# Determine the IP address class from first octet
...
# Decide if the address is public or private
type_of_ip = None
if (
...
# Build the subnet mask in binary then decimal
...
# Convert a single decimal octet to 8-bit binary string
def ip_in_bin(octet):
    m = str(bin(octet))
    m1 = [i for i in m]
    m1 = m1[2::]
...
# AND the IP and mask to get the network (subnet) address
subnet_decimal_list = []
for i in range(4):
...
# Convert first and last host addresses to binary
first_bin_list = []
for i in range(len(ip_list_octet)):
...
# Count total and usable host addresses
available_number_prom = 32 - ip_prefix
available_number = 2**available_number_prom


# Print all calculated network information
