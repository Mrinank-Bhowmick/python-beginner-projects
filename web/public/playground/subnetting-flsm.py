# === Subnetting FLSM · annotated for the pyBegin playground ===
# A beginner-friendly walkthrough — original project by @bim22614.

# Collect network settings from the user
def start_v():
...
# Unpack the returned list into named variables
start_list = start_v()
...
# Check if the network has enough IP addresses
def enough_ip():
...
# Build the subnet mask in binary for each octet
def rules_ip():
    ip_by_octat = []
...
# Build a lookup table mapping prefix to host count
table_of_prefix_value = {}
for i in range(0, 32):
    table_of_prefix_value.update({str(32 - i): 2**i})


# Find the smallest prefix that fits all hosts
...
# Convert one IP octet integer to an 8-bit binary string
def ip_in_bin(octet):
    m = str(bin(octet))
    m1 = [i for i in m]
    m1 = m1[2::]
...
# Split the network address into octets and convert to binary
...
# Calculate network and broadcast addresses for each subnet
