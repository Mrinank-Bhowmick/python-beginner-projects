# === Hand Cricket Game · annotated for the pyBegin playground ===
...
# Entry point — sets up game and calls helpers
def main():
    ...
    try:
        ...
        # Run the coin toss to decide who bats first
        toss_winner = toss()
        if toss_winner == 1:
        ...
        # Play the match and collect final scores
...
# Simulate a coin toss for both players
def toss():
    print("Toss time!")
...
# Run all overs for both players
def play_game(overs, player1_choice, player2_choice, difficulty=1):
    ...
    for over in range(overs):
        ...
        # Show scores at end of each over
        display_scoreboard(player1_score, player2_score, over)

    return player1_score, player2_score


# Handle one player's turn ball by ball
def user_turn(player_score, player_wickets, player_choice, over):
    ...
    while balls < 6 and player_wickets > 0:
        if player_choice == "1":
            # Batting: player enters shot, computer bowls randomly
            player_runs = int(
                input(f"Over {over + 1}, Ball {balls + 1}: Enter your {'shot'} (1-6): ")
            )
            opponent_runs = random.randint(1, 6)
        else:
            # Bowling: player chooses delivery, computer bats randomly
        ...
        # Matching numbers means a wicket or an out
...
# Print the current scoreboard after an over
